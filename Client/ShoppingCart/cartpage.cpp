...
{
    ui->setupUi(this);
    ui->list_item->setStyleSheet("QListWidget{border:1px;border-radius:5px;}"//设置ui列表的样式表
                           "QListWidget::Item{height:80px;}"
                           );
    ui->list_item->setSpacing(2);

    this->tolprice = 0;
    ui->lab_tolprice->setText(QString("总金额: ￥%1").arg(this->tolprice));
    isCached = false;
...
void CartPage::loadCart(){
    ...
    if(flag[0] != "1"){client->error(flag[0],flag.mid(1));return;}
        ...
        QJsonArray res = client->parseResponse(data);//解析返回的报文
        //qDebug()<<res;
        for(int i = 0; i < res.size(); i++){
            ...
            QListWidgetItem *item = new QListWidgetItem(ui->list_item);//单个列表类
