...
void HandleServer::handleRequest(const QString& ip,const qintptr port, const QByteArray data){//识别信息，将报文传给相关函数
       qDebug()<<"HandleServer::handleRequest";
...
void HandleServer::jsonResReady(QString head,QJsonArray res,qintptr port,QString errmsg){//发送反馈信息
...
void HandleServer::handleUserLogin(QJsonObject body, qintptr port){
    ...
    if(flag){
        jsonResReady("1",res,port);//反馈
    }
    else{jsonResReady("3",QJsonArray(),port,"查询用户名失败handleUserLogin！");return;}
}
...
void HandleServer::handleManagerLogin(QJsonObject body, qintptr port){
    ...
    else{jsonResReady("3",QJsonArray(),port,"查询用户名失败handleManagerLogin！");return;}
}

void HandleServer::handleRegister(QJsonObject body, qintptr port){//用户注册
    QString table = "users";
    QJsonArray res;
    if(body.contains("user_name")){
        ...
        QJsonArray search;
        bool fl = sql->selectSth(table,obj,search);//先检索是否注册过
        if(fl){
            if(!search.isEmpty()){
            ...
            jsonResReady("3",QJsonArray(),port,"handleRegister查询用户名失败！");
            return;
        }
    }
    bool flag = sql->insertSth(table,body);//录入数据
    if(flag){
        jsonResReady("1",QJsonArray(),port);
    }
    else{jsonResReady("3",QJsonArray(),port,"录入用户名失败！");return;}
}
...
void HandleServer::k_handleChat(QJsonObject body,qintptr port){
    QString msg;
    QString chat_user_id;
    QJsonArray res;
    if(body.contains("msg")){
        msg = body.value("msg").toString();
    }
    if(body.contains("chat_user_id")){
        chat_user_id = body.value("chat_user_id").toString();
    }
    //直接写入数据库，然后发送更新信号？
